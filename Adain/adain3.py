...
def decoder():
  inputs = tf.keras.layers.Input(shape=(32, 32, 512), batch_size = None)
  x = tf.keras.layers.Conv2D(filters=512, kernel_size=(3,3))( tf.pad(inputs, [[0, 0], [1, 1], [1, 1], [0, 0]],
             mode='REFLECT'))
  x = tf.keras.layers.ReLU()(x)
  x = tf.keras.layers.UpSampling2D()(x)

  x = tf.keras.layers.Conv2D(filters=256, kernel_size=(3,3))(tf.pad(x, [[0, 0], [1, 1], [1, 1], [0, 0]],
             mode='REFLECT'))
  x = tf.keras.layers.ReLU()(x)
  ...
  x = tf.keras.layers.Conv2D(filters=128, kernel_size=(3,3))(tf.pad(x, [[0, 0], [1, 1], [1, 1], [0, 0]],
             mode='REFLECT'))
  x = tf.keras.layers.ReLU()(x)


  x = tf.keras.layers.Conv2D(filters=64, kernel_size=(3,3))(tf.pad(x, [[0, 0], [1, 1], [1, 1], [0, 0]],
             mode='REFLECT'))
  x = tf.keras.layers.ReLU()(x)
  ...
  x = tf.keras.layers.Conv2D(filters=3, kernel_size=(3,3))(tf.pad(x, [[0, 0], [1, 1], [1, 1], [0, 0]],
             mode='REFLECT'))
...
def get_decoder(encoder):
  ...
  # Mirror the encoder
  x = inputs
  for i in reversed(range(len(encoder.layers))):
...
def train_step(content_img, style_img):
  with tf.GradientTape() as tape:
    encoded_content = encoder(tf.keras.applications.vgg19.preprocess_input(content_img*255))
    encoded_style = encoder(tf.keras.applications.vgg19.preprocess_input(style_img*255))
    ...
    encoded_style_loss = loss_net(tf.keras.applications.vgg19.preprocess_input(style_img*255))
...
#encoded_content = encoder(tf.keras.applications.vgg19.preprocess_input(style_tr*255))
#encoded_style = encoder(tf.keras.applications.vgg19.preprocess_input(content_tr*255))

#adain_output = adaptive_instance_normalization(encoded_content,
#                                    encoded_style)
...
"""
style = load_img_Inference("/content/drive/MyDrive/content_images/Wikiart/Action_painting/hans-hofmann_the-wind-1942.jpg")
content = load_img_Inference("/content/drive/MyDrive/content_images/mini_batch/000000000802.jpg")

decoder.load_weights("/content/decoder")

encoded_content = encoder(tf.keras.applications.vgg19.preprocess_input(content*255))
encoded_style = encoder(tf.keras.applications.vgg19.preprocess_input(style*255))
