...
def train_step(X_batch, extractor, training_model, style_targets, content_weight, style_weight, TV_WEIGHT):
    with tf.GradientTape() as tape:

        content_targets = extractor(X_batch*255.0)['content']
        image = training_model(X_batch)
        image = tf.clip_by_value(image, 0.0, 1.0)

        outputs = extractor(255*image, training = True)
...
def train(model, style_layers, content_layers, batch_size, style_path, dataset_path, content_weight, style_weight, TV_WEIGHT, weights_path):
    ...
    for n in range(epochs):
        ...
        for img in train_dataset:
            ...
            loss = train_step(X_batch, extractor, training_model, style_targets, content_weight, style_weight, TV_WEIGHT)
